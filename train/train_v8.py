...
def bn_l1_loss(modules):
    l1 = torch.tensor(0., device=next(modules[0].parameters()).device)
    for m in modules:
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    elif pretrained and opt.ft_pruned:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # 🔧 sparsity loss 추가
                sl_loss = torch.tensor(0., device=device)  # default 0
                if opt.sl_factor > 0:
                    sl_loss = bn_l1_loss(prunable_modules)  # L1 norm of BN weights
                    loss += opt.sl_factor * sl_loss
            ...
            if RANK in [-1, 0]:
                ...
                if opt.sl_factor > 0:
                    sl_mloss = (sl_mloss * i + sl_loss.detach()) / (i + 1)  # ✅ 수정
